...
class AppUIActions {
    ...
    secretin
      ...
      .catch(error => {
        if (error instanceof Errors.UserNotFoundError) {
          ...
          return secretin
            .addSecret('Windows', DEFAULT_SECRET, undefined, 'windows')
            .then(secretId =>
  ...
  generatePassword({ windowsSecretId, data }) {
    ...
    const newData = data.update('fields', fields =>
      fields.push(
        new Immutable.Map({ date: moment().format(), content: newPassword })
      )
    );

    secretin
      .editSecret(windowsSecretId, newData)
